...
# This needs the mutagen and the smbprotocol packages.
# This program also generates a .csv file of all files' metadata including tags.
# For the Samba share path, use IP address rather than server name due to how 
...
music_root = input('Enter folder path for music library (e.g., 192.168.1.75/Music): ')
...
def analyze_file(pathname: str, filename: str):
    ...
    f = smbclient.open_file(fullname, 'rb')
    ...
    f.close()
    return
# End analyze_file function        
        
# Main program
# Recusively walk through all subfolders, and files therein in sorted order
for dirName, subdirList, fileList in smbclient.walk(music_root):
    dir_count += 1
    dirhName = dirName.replace('\\', '/')  # Fix Windows backslashes returned by smbclient
